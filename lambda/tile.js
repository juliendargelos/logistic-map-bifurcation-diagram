...
const width = 256;
const height = 256;
const minimumColor = [0, 0, 0];
const maximumColor = [0, 0, 255];
const backgroundColor = [0, 0, 0];

var handler = async ({ queryStringParameters }) => {
  let { x = '0', y = '0', z = '0', iterations = 20 } = queryStringParameters;
  const range = 1 / Math.pow(2, parseInt(z, 10));
  const scaledIterations = iterations; //Math.min(50000, Math.max(500, iterations / Math.log(range + 1)))
